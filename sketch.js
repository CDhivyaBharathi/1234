...
function setup(){
    ...
    backImage = createSprite((width - 50)/2, (height - 450)/2);
    backImage.addImage(bgImg);
    backImage.scale = 1.5;
  
    //creating bhoviya and adding image to her and setting the cpllider
    bhoviya = createSprite((width - 50)/8,(height - 450),50,50);
   ...
   //Creating a visible ground
    ...
    jButton = createSprite((width - 50)/1.5 + 300,(height - 300),50,50);
...
function draw(){
    ...
    if(gameState === PLAY){
      ...
      if(mousePressedOver(jButton) && bhoviya.collide(vGround) || touches.length>0) {
        jumpSound.play();
    ...
    text(" : "+ keyCount, (width - 50)/10,height - 825);
    text(" : "+ coinCount, (width - 50)/10,height - 800);
    text(" : "+ lifeCount, (width - 50)/10,height - 770);
    text("Press jump to make bhoviya jump and catch, beware of the virus as it will reduce your life",(width - 50)/6,height - 825)
    text("catch 7 keys to win a heart for a life",(width - 50)/3,height - 225)
...
function spawnKeys() {
    ...
    keys.velocityX = -4;
    keys.y = Math.round(random(height - 650,height - 800));
  ...
  function spawnVirus() {
    ...
    if (frameCount % 593 === 0) {
      ...
      virus.y = Math.round(random(height - 550,height - 750));
    ...
    if (frameCount % 459  === 0) {
      ...
      virus1.y = Math.round(random(height - 550,height - 750));
  ...
  function spawnCoins() {
    ...
    if (frameCount % 409 === 0) {
      ...
      coins.y = Math.round(random(height - 600,height - 750));
    ...
    if (frameCount % 623 === 0){
      ...
      coins1.y = Math.round(random(height - 600,height - 750));
